...
*/
self.createDatasetFromDataArrays = function (
  dataArrays,
  labels,
  featureIndices,
  batchSize
...
self.norm2DArray = function (array) {
  ...
  for (var j = 0; j < numFeatures; j++) {
    ...
    stddevs_array[j] = Math.sqrt(
      sum_squares / length - Math.pow(means_array[j], 2)
    );
  }
...
self.fitBetaDistribution = function (counts) {
  ...
  var alphas = tensorCounts
    ...
    .add(tf.scalar(0.1));
  while (true) {
    const numerator = tensorCounts
      .div(tensorCounts.sub(1).add(alphas))
      .sum((axis = 0));
