...
self.createConfusionMatrix = (predictions, ground_truth) =>{
    var confusion_matrix = [[0,0],
                            [0,0]]
    for (var i =0; i < predictions.length; i++) {
        // negative
        if (ground_truth[i] === 0) {
            // negative
            if (predictions[i] === 0) { confusion_matrix[1][1]++ }
            // positive
        ...
        // positive
            ...
            // negative
            if (predictions[i] === 0) { confusion_matrix[1][0]++ }
            //positive
            else { confusion_matrix[0][0]++ }
        }
    }
    ...
    const normed_array = array.map(data_row => 
        {
            ...
            return data_row.map( (value, idx) => {
                if (stddevs_array[idx] === 0) { return 0}
