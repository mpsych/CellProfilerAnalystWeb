...
class UploadHandler {
	...
	getFiles() {
		var file_names = ['example_SETUP.SQL', 'per_object.csv', 'per_image.csv', 'MyTrainingSet.txt'];
		var file_handlers = [];
		var i = 0;
	...
	getDataHandlerandStartingTrainingSet = async function () {
		var data_types = ['column_lines', 'object_data', 'image_data', 'training_data'];
		...
		console.log('dataprover ref');
		console.log(DataProvider);
		var dp = new DataProvider({
			object_data: text_data[1],
			image_data: text_data[2],
			object_columns: column_lines[0],
			image_columns: column_lines[1],
		});
