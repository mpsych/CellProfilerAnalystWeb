...
class mblNetClassifier{
  constructor() {
    ...
    this.model = Classifier.createKnnModel(this.numberFeatures);
  ...
  static createKnnModel  (feature_count)  {
      const model = knnClassifier.create();

      return model;
  }

  static async fitDatasetMblNet(model, net, training_dataset) {
    // load examples into klnnClassifier
    const activation = net.infer(training_dataset.xs, true);
    // Add example to model
    model.addExample(activation, training_dataset.ys);
  ...
  static async basicTrainPromise (model, training_dataset, number_epochs, render_containers=null ) {
      ...
      return new Promise(
          (resolve, reject) => {
              model = training_dataset.forEachAsync(e => fitDatasetMblNet(model, net, e));
          }
      )
