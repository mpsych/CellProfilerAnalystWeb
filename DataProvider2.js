...
self.importScripts('ImageObjectTable.js')
class DataProvider2 {
    ...
    constructor(uniform_data, props) {
        ...
        console.time('start')
        this.data = {
            'object_data' :  new ObjectTable([[1,1,1], [2,2,2]], ["ImageNumber", "ObjectNumber", "wut"]),
            'image_data' :  new ImageTable([[1,1,1], [2,2,2]], ["ImageNumber", "ObjectNumber", "wut"])
        }
        console.time('start')
        var table = new ImageObjectTable(uniform_data, props)
        for (var i = 0; i < uniform_data.image_table.data.length; i++) {
            table.initImageData(i, uniform_data.image_table.data[i])
        }
        console.log(table)
        for (var i = 0; i < uniform_data.object_table.data.length; i++) {
            table.initObjectData(i, uniform_data.object_table.data[i])
        }
